...
import { Play, MessageCircle, ClipboardList, Zap, ChevronRight, RefreshCw } from 'lucide-react';
...
export default function VideosPage() {
  ...
  const filteredVideos = videos.filter((v) => filter === 'all' || v.difficultyLevel === filter);
    ...
    <div className="min-h-screen pb-28">
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {(['all', 'beginner', 'intermediate', 'advanced'] as const).map((f) => (
            ...
            >
              {f === 'all' ? 'すべて' : getDifficultyLabel(f)}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {filteredVideos.map((video, idx) => (
              <motion.div key={video.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
                <Link href={`/videos/${video.id}`} className="group block glass-card overflow-hidden hover:border-primary/30 transition-all p-3">
                  <div className="flex gap-4">
                    <div className="relative w-32 h-20 flex-shrink-0">
                      {video.thumbnailUrl ? (
                        <img src={video.thumbnailUrl} alt={video.title} className="w-full h-full object-cover rounded-xl" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                      <h3 className="font-bold text-sm text-white/90 line-clamp-2 leading-snug group-hover:text-primary transition-colors">{video.title}</h3>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {video.difficultyLevel && (
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-tighter bg-white/5 text-white/60">
                              {getDifficultyLabel(video.difficultyLevel)}
                            </span>
                          )}
                          <span className="text-[10px] text-white/20">{new Date(video.publishedAt).toLocaleDateString('ja-JP')}</span>
                        </div>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-md z-[100]">
        <div className="glass-card p-2 flex justify-between items-center shadow-[0_20px_50px_rgba(0,0,0,0.8)] border-white/10">
          ...
          <Link href="/videos" className="nav-item active px-6 py-3"><Play size={24} /><span className="text-[9px] font-bold uppercase tracking-tighter">動画</span></Link>
