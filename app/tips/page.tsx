...
import { 
  Zap, Play, MessageCircle, ClipboardList, Search, 
...
const CATEGORIES = [
  { id: 'all', name: '縺吶∋縺ｦ', icon: BookOpen },
  { id: 'technique', name: '繝・け繝九ャ繧ｯ', icon: Zap },
  { id: 'training', name: '繝医Ξ繝ｼ繝九Φ繧ｰ', icon: Dumbbell },
  { id: 'mental', name: '繝｡繝ｳ繧ｿ繝ｫ', icon: Brain },
  { id: 'equipment', name: '陬・ｙ繝ｻ驕灘・', icon: Shield },
];
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        <header className="space-y-2">
          <h2 className="text-2xl font-display font-bold text-white flex items-center gap-2">
            繝繝ｼ繝悶・讌ｵ諢・<Sparkles className="text-primary w-5 h-5" />
          </h2>
          <p className="text-white/40 text-sm">蜍慕判縺九ｉ謚ｽ蜃ｺ縺輔ｌ縺溷ｮ溯ｷｵ逧・ユ繧ｯ繝九ャ繧ｯ</p>
        </header>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="h-64 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-white/20 text-sm font-medium">遏･諱ｵ縺ｮ豕｢蠖｢繧定ｪｭ縺ｿ蜿悶ｊ荳ｭ...</p>
            </div>
          ) : error ? (
            <div className="glass-card p-8 text-center space-y-4 border-red-500/20 bg-red-500/5">
              <p className="text-red-400 font-medium">{error}</p>
              <button onClick={() => location.reload()} className="text-xs text-primary font-bold">蜀崎ｩｦ陦・/button>
            </div>
          ) : filteredTips.length === 0 ? (
            <div className="glass-card p-12 text-center">
              <p className="text-white/20 text-sm font-medium">隧ｲ蠖薙☆繧九さ繝・′隕九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <div className={`flex items-center gap-2 ${theme.color} text-[10px] font-black uppercase tracking-widest`}>
                          <ThemeIcon size={12} />
                          {CATEGORIES.find(c => c.id === tip.category)?.name || '譛ｪ蛻・｡・}
                        </div>
